...
if (fread_int_values(fid, ibuf, 4) != 4) 
   {
    mexPrintf("error in 'load_data_from_file':  file appears to be empty!\n");
    return(-1);
   }
if (ibuf[0] != 4 && ibuf[0] != 8)
   {
    CPPSIM_OLD_FILE_FLAG = 1;
    mexPrintf("...reading in old version of CppSim simulation file...\n");
    rewind(fid);
    if (fread_int_values(fid, ibuf, 4) != 4) 
       {
       mexPrintf("error in 'load_data_from_file':  file read error\n");
       return(-1);
       }
    if (ibuf[0] != 4 && ibuf[0] != 8)
      {
       mexPrintf("error in 'load_data_from_file':  file is corrupted\n");
       return(-1);
      }
   }
rewind(fid);
