...
% Usage: plot_psd(x, exp) or plot_psd(x, exp, plot_args)
...
function [] = plot_psd(x,exp,plot_args)
...
for i = 1:nsp
    ...
    if nargin > 2
	plot_psd1(x,cplot,plot_args);
    else
	plot_psd1(x,cplot);
    end
    if i == 1
       title('One-Sided Power Spectral Density (V^2/Hz or A^2/Hz)')
    end
end
