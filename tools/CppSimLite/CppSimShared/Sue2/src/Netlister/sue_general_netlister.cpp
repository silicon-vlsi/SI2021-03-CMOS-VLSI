...
//first_bus_node = evaluate_bus_nodes("n1<2:0>,n3<3:0>,am,pm,r<1:0>");
//print_bus_nodes(first_bus_node);
//////////////////
...
remove_intermediate_wires(first_module);
transfer_bus_node_names(first_module);
check_wire_groups(first_module);


remove_unnamed_wire_prefix(first_module);
// print_keyinfo_modules(first_module); exit(1);
...
if (strcmp(simulator_name,"hspice") == 0)
  {
   create_bus_nodes_for_modules_and_instances(first_module);
   print_hspice_netlist(outfile,first_module,print_top_subckt_flag);
   printf("Spice netlisting of cell '%s' completed with no errors\n",
          top_sue_module_name);
  }
 else
   {
