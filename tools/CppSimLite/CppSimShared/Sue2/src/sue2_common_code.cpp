...
#else
     return(mkdir(dir_name,S_IRWXU|S_IRGRP|S_IXGRP));
#endif
   }
     ...
     while (cur_module2 != NULL)
         ...
         if (strcmp(cur_module->name,cur_module2->name) == 0)
	   {
	     if (flag == 0)
	       {
		 flag = 1;
	         printf("Warning:  duplicate module names exist\n");
                 printf("----------------------------------\n");
	       }
	     if (cur_module->shared_library_flag == 2)
	        printf("    %s in '%s:Private'\n",cur_module->name,
                        cur_module->library_path);
             else        
	        printf("    %s in '%s' ",cur_module->name,
                        cur_module->library_path);
	     if (cur_module2->shared_library_flag == 2)
                printf("and '%s:Private'\n", cur_module2->library_path);
             else
                printf("and '%s'\n", cur_module2->library_path);
	   }
     ...
     while (cur_module2 != NULL)
         ...
         if (strcmp(cur_module->name,cur_module2->name) == 0)
	   {
	     if (flag == 0)
	       {
		 flag = 1;
	         printf("Error:  duplicate module names exist\n");
                 printf("----------------------------------\n");
	       }
	     if (cur_module->shared_library_flag == 2)
	        printf("    %s in '%s:Private'\n",cur_module->name,
...
if (flag == 1)
  {
    printf("----------------------------------\n\n");
    printf("   --> You must correct this issue by removing the appropriate\n");
    printf("       .sue files from the above libraries\n");
    printf("       EVERY MODULE NAME MUST BE UNIQUE WITHIN ALL OF THE\n");
    printf("                      SUE2 LIBRARIES\n\n");
    printf("   ******** Netlisting NOT performed due to above error ********\n");
    exit(1);
  }
}
...
MODULE *extract_sue_modules(const char *sue_lib_filename, PARAMETER *param_list)
   {
    return(extract_sue_modules(sue_lib_filename, param_list, "normal"));
   }
...
MODULE *first_module;
FILE *sue_lib_file;
char dirname[1000],base_dir[1000],full_dir[1000],full_dir_temp[1000];;
char sue_filename[1000];
DIR *dir, *dir_temp;
struct dirent *entry;
...
while (fscanf(sue_lib_file,"%s",dirname) == 1)
  {
    ...
    if ((dir = opendir(full_dir)) == NULL) 
       ...
       {
        sprintf(base_dir,"%s",cppsimshared_home_dir);
        sprintf(full_dir_temp,"%s/SueLib/%s",base_dir,dirname);
        if ((dir_temp = opendir(full_dir_temp)) != NULL) 
           {
	    closedir(dir_temp);
            shared_library_flag = 2;
	  }
       }
    ...
    while ((entry = readdir(dir)) != NULL) 
	 ...
	 {
	  sprintf(sue_filename,"%s/%s",full_dir,entry->d_name);
          first_module = add_sue_module(first_module,sue_filename,
				     dirname,param_list,shared_library_flag,
                                     control_string);
	 }
       }
    closedir(dir);
...
char sue_filename[1000];
DIR *dir, *dir_cell;
struct dirent *entry,*entry_cell;
const char *cppsim_home_dir_const = NULL;
const char *cppsimshared_home_dir_const = NULL;
char cppsim_home_dir[MAX_CHAR_LENGTH], cppsimshared_home_dir[MAX_CHAR_LENGTH];
int shared_library_flag;
...
// Scan through the SueLib directories and files in the Import_Export directory
sprintf(base_dir,"%s/Import_Export",cppsim_home_dir);
if ((dir = opendir(base_dir)) == NULL) 
  {
    printf("error in 'extract_sue_modules_archive':\n");
    printf("   cannot open Import_Export directory:\n");
    printf("   %s\n",base_dir);
    exit(1);
  }
else
...
sprintf(base_dir,"%s/Import_Export/SueLib",cppsim_home_dir);
if ((dir = opendir(base_dir)) == NULL) 
  {
    printf("error in 'extract_sue_modules_archive':\n");
    printf("   cannot open Import_Export/SueLib directory:\n");
    printf("   %s\n",base_dir);
    exit(1);
  }
...
while ((entry = readdir(dir)) != NULL) 
    {
     ...
     sprintf(full_dir,"%s/%s",base_dir,dirname);
     if ((dir_cell = opendir(full_dir)) == NULL) 
        {
         printf("error in 'extract_sue_modules_archive':\n");
         printf("   cannot open Import_Export/SueLib sub-directory:\n");
         printf("   %s\n",full_dir);
         exit(1);
        }
     chdir(full_dir);

     while ((entry_cell = readdir(dir_cell)) != NULL) 
        {
         if (is_sue_file(entry_cell->d_name))
	    {
	    sprintf(sue_filename,"%s/%s",full_dir,entry_cell->d_name);
            first_module = add_sue_module(first_module,sue_filename,
				     dirname,param_list,shared_library_flag,
                                     "normal");
	    }
...
if ((sue_lib_file = fopen(sue_lib_filename,"r")) == NULL)
  {
   printf("error in 'extract_sue_modules_oldversion':  can't open sue.lib file:  '%s'\n",
            sue_lib_filename);
   exit(1);
 }
...
sprintf(full_dir,"%s/SueLib",base_dir);
if ((dir = opendir(full_dir)) == NULL) 
   {
     fclose(sue_lib_file);
     return(NULL);
   ...
   {
     printf("\n... Assuming the Old Version of CppSim ...\n\n");
   }
while (fscanf(sue_lib_file,"%s",dirname) == 1)
  {
    // Open the current directory named in the sue_lib_file
    ...
    if ((dir = opendir(full_dir)) == NULL) 
       {
        printf("error in 'extract_sue_modules_oldversion':\n");
        printf("    Could not open directory %s\n",full_dir);
	printf("    contained in the sue.lib file '%s'\n",sue_lib_filename);
        exit(1);
       }
    ...
    while ((entry = readdir(dir)) != NULL) 
       {
       sprintf(full_sue_filename,"%s/%s",full_dir,entry->d_name);
       if (is_sue_file(entry->d_name))
          first_module = add_sue_module(first_module,full_sue_filename,
				     dirname,param_list,0);
       }
    closedir(dir);
  }
...
while (cur_module != NULL)
   {
    if (cur_module->active_sue_module_flag == 1 ||
        (strcmp(simulator_name,"cppsim") == 0 && 
         is_conversion_module(cur_module) == 1))
       {
...
if (cur_module == NULL)
   {
     printf("error in 'is_conversion_module':  cur_module is NULL\n");
     exit(1);
   }

if (strcmp(cur_module->name,"cpp_internal_int_to_double_convert") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_to_int_convert") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_to_bool_convert") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_interp_to_bool_convert") == 0 ||
    strcmp(cur_module->name,"cpp_internal_bool_to_double_interp_convert") == 0)
      return(1);
else if (strcmp(cur_module->name,"cpp_internal_int_to_double_convert_cpp") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_to_int_convert_cpp") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_to_bool_convert_cpp") == 0 ||
    strcmp(cur_module->name,"cpp_internal_double_interp_to_bool_convert_cpp") == 0 ||
    strcmp(cur_module->name,"cpp_internal_bool_to_double_interp_convert_cpp") == 0)
      return(1);
else
      return(0);
}
...
while (cur_wire != NULL)
   {
     if (strncmp(cur_wire->node_name,"UNNAMED_WIRE_",13) == 0)
        printf("*** wire:  node '%s', %d %d %d %d (wire_group: %d)\n",
	    cur_wire->node_name, cur_wire->x1, cur_wire->y1,
	    cur_wire->x2, cur_wire->y2, cur_wire->wire_group);
     cur_wire = cur_wire->next;
   }
}

// returns 1 if bus (i.e., n0<2:0>
//         2 if bundle (i.e., n0,n1,n2)
//         0 otherwise
...
if (node_name[i] == '<')
  {
    if (i == 0)
      {
       printf("error in 'check_if_bus_or_bundle':  bus name is invalid\n");
...
// returns 1 if bus signal (i.e., n0<2:0> or n0<3>
//         0 otherwise
...
if (node_name[i] == '<')
  {
    if (i == 0)
      {
       printf("error in 'check_if_bus_signal':  bus name is invalid\n");
       printf("   -> in this case, bus name = '%s'\n",node_name);
       exit(1);
      }
    count = 0;
...
{
MODULE *cur_module;
WIRE *cur_wire,*prev_wire,*next_wire;
...
while (cur_module != NULL)
   {
     ...
     while (cur_wire != NULL)
       {
         if (cur_wire->connecting_instance == NULL)
	   {
	     if (prev_wire == NULL)
	       {
		next_wire = cur_wire->next;
		delete cur_module->first_wire;
 	        cur_module->first_wire = next_wire;
		cur_wire = NULL;
		prev_wire = NULL;
	       }
             else
	       {
		 prev_wire->next = cur_wire->next;
                 delete(cur_wire);
                 cur_wire = prev_wire;
	       }
	   }
...
INSTANCE *cur_instance;
NODES *cur_node;
char temp_str[MAX_NODE_NAME_LENGTH];
...
while (cur_module != NULL)
   {
	 ...
	 while (cur_node != NULL)
	   {
	     if (strncmp(cur_node->name,"UNNAMED_WIRE_",13) == 0)
	       {
		 strcpy(temp_str,cur_node->name);
		 strcpy(cur_node->name,&temp_str[13]);
	       }           
...
while (cur_module != NULL)
   {
     ...
     while (cur_wire != NULL)
       {
	 if (strcmp(cur_wire->bus_node_name,"NULL") != 0)
	   {
	     strcpy(cur_wire->node_name,cur_wire->bus_node_name);
	   }
	 cur_wire = cur_wire->next;
       }
     cur_module = cur_module->next;
...
j = 0;
for (i = 0; sig_name[i] != '\0'; i++)
   {
     if (sig_name[i] != ' ' && sig_name[i] != '<')
        base_name[j++] = sig_name[i];
     if (sig_name[i] == '<')
        break;
   }
base_name[j] = '\0';
...
int cur_wire_group, min_ind, max_ind, first, last, count;
char base_name[MAX_NODE_NAME_LENGTH], temp_name[MAX_NODE_NAME_LENGTH];
BUS_NODE *cur_bus_node;
...
while (cur_module != NULL)
   {
     ...
     while (cur_wire != NULL)
       {
         if (strncmp("UNNAMED_WIRE_",cur_wire->node_name,13) == 0)
	   {
	     cur_wire_group = cur_wire->wire_group;
             cur_wire2 = cur_wire->next;
	     while (cur_wire2 != NULL)
	       {
		 if (cur_wire2->wire_group == cur_wire_group)
		   {
		     if (strcmp(cur_wire->node_name,cur_wire2->node_name) != 0)
		       {
			 if (check_if_bus_or_bundle(cur_wire->node_name) != 0 &&
                             check_if_bus_or_bundle(cur_wire2->node_name) != 0)
			   {
                             
			   printf("error in 'check_wire_groups':  mismatched bus width on an unnamed wire\n");
                           printf("   -> in module '%s', please check connection between\n",
                                       cur_module->name);
			   if (cur_wire->connecting_instance != NULL && cur_wire->connecting_instance_terminal != NULL)
			     {
                             printf("       instance '%s', terminal '%s'  and\n",cur_wire->connecting_instance->name,
				cur_wire->connecting_instance_terminal->mod_node_name);
			     }
			   if (cur_wire2->connecting_instance != NULL && cur_wire2->connecting_instance_terminal != NULL)
			     {
                             printf("       instance '%s', terminal '%s'\n",cur_wire2->connecting_instance->name,
                                cur_wire2->connecting_instance_terminal->mod_node_name);
			     }
			   exit(1);
			   }
                         else if (check_if_bus_or_bundle(cur_wire->node_name) != 0)
			   {
			   strcpy(cur_wire->node_name,cur_wire2->node_name);
	                   if (cur_wire->connecting_instance_terminal == NULL)
	                      {
	                      printf("error in 'check_wire_groups':\n");
		              printf("  **** cur_wire->connecting_instance_terminal is NULL for wire '%s'!\n",cur_wire->node_name);
		              printf("       -> this should never happen\n");
	                      }
                           strcpy(cur_wire->connecting_instance_terminal->name,cur_wire->node_name);
			   }
                         else if (check_if_bus_or_bundle(cur_wire2->node_name) != 0)
			   {
			   strcpy(cur_wire2->node_name,cur_wire->node_name);
	                   if (cur_wire2->connecting_instance_terminal == NULL)
	                      {
	                      printf("error in 'check_wire_groups':\n");
		              printf("  **** cur_wire2->connecting_instance_terminal is NULL for wire '%s'!\n",cur_wire2->node_name);
		              printf("       -> this should never happen\n");
	                      }
                           strcpy(cur_wire2->connecting_instance_terminal->name,cur_wire2->node_name);
		           }
		       }
		   }
         ...
         else if (strcmp("INVALID",cur_wire->node_name) == 0) // invalid and no bus
	   {
	    printf("error in 'check_wire_groups':  unlabeled wire\n");
            printf("   -> in module '%s'\n",cur_module->name);
	    if (cur_wire->connecting_instance != NULL && cur_wire->connecting_instance_terminal != NULL)
	      printf("      instance '%s', terminal '%s' needs its connecting wire labeled\n",
		     cur_wire->connecting_instance->name, cur_wire->connecting_instance_terminal->mod_node_name);
	    else
	      {
		printf("  **** error:  cur_wire->connecting_instance is NULL for wire '%s'!\n", cur_wire->node_name);
		printf("       -> this should never happen\n");
	      }
            exit(1);
	   }
         else if (strncmp("INVALID_",cur_wire->node_name,8) == 0) // invalid and bus
	   {
	     cur_wire_group = cur_wire->wire_group;
             ...
             max_ind = 0;
	     count = 0;
	     strcpy(base_name,"NULL"); 
	     while (cur_wire2 != NULL)
	       {
		 if (cur_wire2 == cur_wire)
		   {
		     cur_wire2 = cur_wire2->next;
		     continue;
		   }
		 if (cur_wire2->wire_group == cur_wire_group)
		   {
		    count++;
		    if (strncmp(cur_wire2->node_name,"INVALID",7) == 0 || // can't handle more than one invalid wire
                        check_if_bus_or_bundle(cur_wire2->node_name) == 2 || // can't handle bundles
                        check_if_bus_signal(cur_wire2->node_name) == 0) // must be bus signals (i.e., n0<3>, n0<3:0>)
		       {
	               printf("error in 'check_wire_groups':  unlabeled wire bus\n");
                       printf("   -> in module '%s'\n",cur_module->name);
	               if (cur_wire->connecting_instance != NULL && cur_wire->connecting_instance_terminal != NULL)
	                   printf("      instance '%s', terminal '%s' needs its connecting wire labeled\n",
		              cur_wire->connecting_instance->name, cur_wire->connecting_instance_terminal->mod_node_name);
			 ...
			 {
			   determine_base_name(cur_wire2->node_name,base_name);
			   if (strcmp(base_name,"NULL") == 0)
			     {
 	                      printf("error in 'check_wire_groups':  invalid node name '%s' in bus\n",
                                      cur_wire2->node_name);
                              printf("   -> in module '%s'\n",cur_module->name);
	                      if (cur_wire2->connecting_instance != NULL && cur_wire2->connecting_instance_terminal != NULL)
	                         printf("      instance '%s', terminal '%s' has an invalid bus label\n",
		                      cur_wire2->connecting_instance->name, cur_wire2->connecting_instance_terminal->mod_node_name);
	                      else
	                         {
		                 printf("  **** error:  cur_wire2->connecting_instance is NULL for wire '%s'!\n",cur_wire2->node_name);
		                 printf("       -> this should never happen\n");
	                         }
                              exit(1);
			     }
			 ...
			 {
			   determine_base_name(cur_wire2->node_name,temp_name);
			   if (strcmp(base_name,temp_name) != 0)
			     {
 	                      printf("error in 'check_wire_groups':  invalid bus labels\n");
                              printf("   with base node names '%s' and '%s'\n",base_name,temp_name);
                              printf("   -> in module '%s', please check connection between\n",
                                       cur_module->name);
			      if (cur_wire->connecting_instance != NULL && cur_wire->connecting_instance_terminal != NULL)
			        {
		    ...
		    first = get_first_bus_val(cur_wire2->node_name);
		    last = get_last_bus_val(cur_wire2->node_name);
                    if (first < min_ind)
			 min_ind = first;
                    if (last < min_ind)
                         min_ind = last;
	     ...
	     if (get_bus_length(cur_wire->node_name) != (max_ind - min_ind + 1) || count == 0)
	       {
	        printf("error in 'check_wire_groups':  unlabeled wire bus is mismatched in size to terminal\n");
                printf("   -> in module '%s'\n",cur_module->name);
	        if (cur_wire->connecting_instance != NULL && cur_wire->connecting_instance_terminal != NULL)
	            printf("      instance '%s', terminal '%s' is attached to an unlabeled wire\n",
		              cur_wire->connecting_instance->name, cur_wire->connecting_instance_terminal->mod_node_name);
	        else
	           {
		    printf("  **** error:  cur_wire->connecting_instance is NULL for wire '%s'!\n",cur_wire->node_name);
		...
		printf("      size of terminal bus: %d,  estimated size of connecting wire: %d\n",
		       get_bus_length(cur_wire->node_name),max_ind-min_ind+1);
                exit(1);
	       }

             sprintf(cur_wire->node_name,"%s<%d:%d>",base_name,max_ind,min_ind);
	     if (cur_wire->connecting_instance_terminal == NULL)
	       {
	        printf("error in 'check_wire_groups':\n");
		printf("  **** cur_wire->connecting_instance_terminal is NULL for wire '%s'!\n",cur_wire->node_name);
	 ...
	 else // cur_wire is labeled
	   {
	     if (check_if_bus_or_bundle(cur_wire->node_name) != 0)
	       {
		if (cur_wire->connecting_instance_terminal == NULL || cur_wire->connecting_instance == NULL)
		  {
	           printf("error in 'check_wire_groups':\n");
		   printf("  **** cur_wire->connecting_instance(_terminal) is NULL for wire '%s'!\n",cur_wire->node_name);
		   exit(1);
		  }
	        if (check_if_bus_or_bundle(cur_wire->node_name) == 1)
		  {
		    if (check_if_bus_or_bundle(cur_wire->connecting_instance_terminal->mod_node_name) == 0)
		      {
		       if (check_if_bus_or_bundle(cur_wire->connecting_instance->name) == 1)
			 {
                          if (get_bus_length(cur_wire->node_name) != get_bus_length(cur_wire->connecting_instance->name))
			    {
	                     printf("error in 'check_wire_groups':  wire bus is mismatched in size to arrayed terminal\n");
                             printf("   -> in module '%s'\n",cur_module->name);
	                     printf("      instance '%s', terminal '%s' is attached to wire '%s'\n",
		                cur_wire->connecting_instance->name, 
                                cur_wire->connecting_instance_terminal->mod_node_name,cur_wire->node_name);
		             printf("      size of arrayed instance bus: %d,  size of connecting wire: %d\n",
                                         get_bus_length(cur_wire->connecting_instance->name),get_bus_length(cur_wire->node_name));
                             exit(1);
			    }
			 }
		       else
			 {
	                  printf("error in 'check_wire_groups':  wire bus is mismatched in size to terminal\n");
                          printf("   -> in module '%s'\n",cur_module->name);
	                  printf("      instance '%s', terminal '%s' is attached to wire '%s'\n",
		              cur_wire->connecting_instance->name, 
                              cur_wire->connecting_instance_terminal->mod_node_name,cur_wire->node_name);
		          printf("      size of terminal bus: 1,  size of connecting wire: %d\n",
                                         get_bus_length(cur_wire->node_name));
                          exit(1);
			 }
		      }
		    else if (get_bus_length(cur_wire->node_name) != get_bus_length(cur_wire->connecting_instance_terminal->mod_node_name))
		      {
	               printf("error in 'check_wire_groups':  wire bus is mismatched in size to terminal\n");
                       printf("   -> in module '%s'\n",cur_module->name);
	               printf("      instance '%s', terminal '%s' is attached to wire '%s'\n",
		              cur_wire->connecting_instance->name, 
                              cur_wire->connecting_instance_terminal->mod_node_name,cur_wire->node_name);
		       printf("      size of terminal bus: %d,  size of connecting wire: %d\n",
		          get_bus_length(cur_wire->connecting_instance_terminal->mod_node_name),
                                         get_bus_length(cur_wire->node_name));
                       exit(1);
		      }
		  }
		  ...
		  {
		    if (cur_wire->connecting_instance_terminal->first_bus_node == NULL)
		      cur_wire->connecting_instance_terminal->first_bus_node = evaluate_bus_nodes(cur_wire->node_name);
		    count = 0;
		    cur_bus_node = cur_wire->connecting_instance_terminal->first_bus_node;
		    while (cur_bus_node != NULL)
		      {
			count++;
			cur_bus_node = cur_bus_node->next;
		      }
		    if (check_if_bus_or_bundle(cur_wire->connecting_instance_terminal->mod_node_name) == 0 &&
                        check_if_bus_or_bundle(cur_wire->connecting_instance->name) == 1)
			 {
                          if (count != get_bus_length(cur_wire->connecting_instance->name))
			    {
	                     printf("error in 'check_wire_groups':  wire bus is mismatched in size to terminal\n");
                             printf("   -> in arrayed module '%s'\n",cur_module->name);
	                     printf("      instance '%s', terminal '%s' is attached to wire '%s'\n",
		                cur_wire->connecting_instance->name, 
                                cur_wire->connecting_instance_terminal->mod_node_name,cur_wire->node_name);
		             printf("      size of arrayed instance bus: %d,  size of connecting wire: %d\n",
                                         get_bus_length(cur_wire->connecting_instance->name),count);
                             exit(1);
			    }
			 }
		    else if (count != get_bus_length(cur_wire->connecting_instance_terminal->mod_node_name))
		      {
	               printf("error in 'check_wire_groups':  wire bus is mismatched in size to terminal\n");
                       printf("   -> in module '%s'\n",cur_module->name);
	               printf("      instance '%s', terminal '%s' is attached to wire '%s'\n",
...
return(0); // will never reach here, but need to include to avoid C++ compile warning
}
...
INSTANCE *cur_instance;
NODES *cur_node;
char node_name[MAX_NODE_NAME_LENGTH], temp_name[MAX_NODE_NAME_LENGTH];
int count, bus_or_bundle_flag;
int x, y, first, last;
WIRE *found_wire,*temp_wire;
...
while (cur_module != NULL)
   ...
   while (cur_instance != NULL)
       ...
       while (cur_node != NULL)
	 {
	  ...
	  found_wire = determine_connecting_wire_node_name(cur_module,x,y,node_name);
	  if (found_wire != NULL)
	    {
	     if (found_wire->connecting_instance != NULL)
	       {
		 temp_wire = cur_module->first_wire;
		 if (temp_wire == NULL)
		   {
		     printf("error in 'connect_instance_nodes': cur_module->first_wire is NULL!\n");
		     printf("  -> this should never happen!\n");
		     exit(1);
		   }
		 while (temp_wire->next != NULL)
		     temp_wire = temp_wire->next;

                 temp_wire->next = init_wire(found_wire->x1, found_wire->y1,
                                             found_wire->x2, found_wire->y2);
                 strcpy(temp_wire->next->node_name,found_wire->node_name);
                 temp_wire->next->wire_group = found_wire->wire_group;
                 found_wire = temp_wire->next;
	       }
             found_wire->name_is_valid_flag = 1;
             found_wire->connecting_instance = cur_instance;
	     found_wire->connecting_instance_terminal = cur_node;
	    }
	  if (strncmp(node_name,"UNNAMED_WIRE_",13) == 0)
	    {
	      bus_or_bundle_flag = check_if_bus_or_bundle(cur_node->mod_node_name);
	      if (bus_or_bundle_flag == 2)
		{
		  printf("error in 'connect_instance_nodes': invalid node name for module terminal\n");
                  printf("   -> for module '%s', node name '%s' is invalid\n",
			 cur_instance->mod_name,cur_node->mod_node_name);
		  exit(1);
		}
	      else if (bus_or_bundle_flag == 1)
		{
		  first = get_norm_first_bus_val(cur_node->mod_node_name);
		  last = get_norm_last_bus_val(cur_node->mod_node_name);
		  if (first > last)
		      sprintf(temp_name,"%s<%d:%d>",node_name,first,last);
                  else
		      sprintf(temp_name,"%s<%d:%d>",node_name,last,first);
		  strcpy(node_name,temp_name);
		  if (found_wire == NULL)
		    {
		     printf("error in 'connect_instance_nodes': found_wire is NULL!\n");
		     printf("   -> this should never happen\n");
		     exit(1);
		    }
		  strcpy(found_wire->bus_node_name,node_name);
		}
	      
	      // printf("inst name: '%s', inst->mod name: '%s'\n",
              //    node_name,cur_node->mod_node_name);
	      // zippy7
	    }
	  else if (strcmp(node_name,"INVALID") == 0)
	    {
	      bus_or_bundle_flag = check_if_bus_or_bundle(cur_node->mod_node_name);
	      if (bus_or_bundle_flag == 2)
		{
	      ...
	      else if (bus_or_bundle_flag == 1)
		{
		  first = get_norm_first_bus_val(cur_node->mod_node_name);
		  last = get_norm_last_bus_val(cur_node->mod_node_name);
		  sprintf(temp_name,"INVALID_<%d:%d>",first,last);
		  strcpy(node_name,temp_name);
		  if (found_wire == NULL)
		    {
		     printf("error in 'connect_instance_nodes': found_wire is NULL!\n");
	  ...
	  if (strcmp(node_name,"NULL") == 0 ||
	      ...
	      while(1)
		{
		 ...
		 if (check_if_node_name_is_available(cur_module,node_name))
		   {
	            bus_or_bundle_flag = check_if_bus_or_bundle(cur_node->mod_node_name);
	            if (bus_or_bundle_flag == 2) // bundle
		      {
		      printf("error in 'connect_instance_nodes': invalid node name for module terminal\n");
                      printf("   -> for module '%s', node name '%s' is invalid\n",
			 cur_instance->mod_name,cur_node->mod_node_name);
		      exit(1);
	            ...
	            else if (bus_or_bundle_flag == 1) // bus
		      {
		      first = get_norm_first_bus_val(cur_node->mod_node_name);
		      last = get_norm_last_bus_val(cur_node->mod_node_name);
		      if (first > last)
		      ...
		      strcpy(node_name,temp_name);
	            ...
	            sprintf(cur_node->name,"%s",node_name);
	            break;
		   }
		}
              if (strcmp(node_name,"FOUND_TERM") == 0) // connect other terminals
		{
		  //		  printf("FOUND_TERM version of label_connecting_terminals: '%s'\n",node_name);
		  label_connecting_terminals_or_globals(cur_module,x,y,node_name);
		}
	    }
	  else 
	    {
	      //      printf("regular version of label_connecting_terminals: '%s'\n",node_name);
	      label_connecting_terminals_or_globals(cur_module,x,y,node_name);
	    }
	  cur_node = cur_node->next;
	 }
...
while (cur_wire != NULL)
   {
	...
	while (cur_wire2 != NULL)
	  {
	   if (compare_wire_or_term_name(cur_wire2->node_name,cur_node_name) == 0)
	     {
	       found_name_flag = 1;
	       break;
	     }
	  ...
	  {
	    sprintf(cur_wire->node_name,"UNNAMED_WIRE_%s",cur_node_name);
	    break;
	  }
	else
	    continue;
...
MODULE *cur_module;
INSTANCE *cur_instance;
WIRE *cur_wire, *cur_wire2, *temp_wire;
int x1, y1, x2, y2;
int wire_match_flag, name_mismatch_flag;
int count, prev_count, node_label_count;
int wire_group, bad_wire_group;
char base_name[MAX_NODE_NAME_LENGTH];
char base_name2[MAX_NODE_NAME_LENGTH];
...
while (cur_module != NULL)
   {
	 ...
	 */
	 ...
	 if (cur_wire->wire_group < 0)
	    cur_wire->wire_group = wire_group++;
	 ...
	 while (cur_wire2 != NULL)
	   {
	    ...
	    if (wire_match_flag == 1)
		  ...
		  if (strcmp(cur_wire2->node_name,cur_wire->node_name) != 0 &&
                     cur_wire2->name_is_valid_flag != 1 &&
		      strcmp(cur_wire2->node_name,"INVALID") != 0)
		      {
			if (strcmp(cur_wire->node_name,"INVALID") != 0)
			  {
			    if (check_if_bus_or_bundle(cur_wire->node_name) != 2 &&
                                check_if_bus_or_bundle(cur_wire2->node_name) != 2)
			      {
                               determine_base_name(cur_wire->node_name,base_name);
                               determine_base_name(cur_wire2->node_name,base_name2);
			       if (strcmp(base_name,base_name2) != 0)
				 {
   		                  printf("error in 'connect_wires_to_themselves':\n");
				  printf("  -> in module '%s'\n",cur_module->name);
		                  printf("     wire has already been associated with a different node name!\n");
		                  printf("     wire named '%s' and '%s'\n",cur_wire->node_name,cur_wire2->node_name);
		                  exit(1);
				 }
			      }
			  }
                        strcpy(cur_wire2->node_name,"INVALID");
		      }
		  }
	       else
		 {
		  strcpy(cur_wire2->node_name,cur_wire->node_name);
	       ...
	       if (cur_wire2->wire_group < 0)
		  cur_wire2->wire_group = cur_wire->wire_group;
	       else if (cur_wire2->wire_group != cur_wire->wire_group)
		 {
		   bad_wire_group = cur_wire2->wire_group;
		   temp_wire = cur_module->first_wire;
		   while (temp_wire != NULL)
		     {
		       if (temp_wire->wire_group == bad_wire_group)
			  temp_wire->wire_group = cur_wire->wire_group;
		       temp_wire = temp_wire->next;
		     }
		 }
	      } 
...
for (i = 0; name[i] != '\0' && wire_or_term_name[i] != '\0'; i++)
   {
     if (name[i] != wire_or_term_name[i])
        break;
   }
if (name[i] == '\0')
   {
     if (wire_or_term_name[i] == '\0')
       return(0);
     else if (wire_or_term_name[i] == '<')
       return(0);
   }

sprintf(temp_name,"UNNAMED_WIRE_%s",name);

for (i = 0; temp_name[i] != '\0' && wire_or_term_name[i] != '\0'; i++)
   {
     if (temp_name[i] != wire_or_term_name[i])
        break;
   }
if (temp_name[i] == '\0')
   {
...
while (cur_wire != NULL)
   {
   if (compare_wire_or_term_name(cur_wire->node_name,name) == 0)
      return(0);
...
while (cur_instance != NULL)    
   {
    ...
    while (cur_node != NULL)
      {
	if (compare_wire_or_term_name(cur_node->name,name) == 0)
	  return(0);
	cur_node = cur_node->next;
      }
    cur_instance = cur_instance->next;
...
while (cur_module != NULL)
   {
     ...
     while (cur_node != NULL)
       {
	 if (cur_node->first_bus_node == NULL)
	   {
           if (check_if_bus_or_bundle(cur_node->name) != 0)
	      {
                cur_node->first_bus_node = evaluate_bus_nodes(cur_node->name);
	      }
	   }
	 cur_node = cur_node->next;
       }
	 ...
	 while (cur_node != NULL)
	   {
              ...
              if (check_if_bus_or_bundle(cur_node->name) != 0)
		 {
	      ...
	      else if (check_if_bus_or_bundle(cur_node->mod_node_name) == 1)
	         {
	         bus_length = get_bus_length(cur_node->mod_node_name);
	         for (i = 0; i < bus_length; i++)
		   {
		     if (cur_node->first_bus_node == NULL)
		       {
	                cur_node->first_bus_node = init_bus_node(cur_node->name);
	                cur_bus_node = cur_node->first_bus_node;
	               }
	            else
	               {
	                cur_bus_node->next = init_bus_node(cur_node->name);
	                cur_bus_node = cur_bus_node->next;
	               }
		   }
	         }
...
int i, inst_array_length, inst_count, inst_index, inst_end, inst_inc;
char inst_basename[MAX_NODE_NAME_LENGTH];
...
if (cur_module == NULL)
   {
     printf("error in 'print_hspice_netlist': no top module!\n");
     exit(1);
   }
fprintf(fp,"***** Spice Netlist for Cell '%s' *****\n\n",cur_module->name);
...
while (cur_module != NULL)
  {
    ...
    if (cur_module->top_module_flag != 1 ||
       (cur_module->top_module_flag == 1 && print_top_subckt_flag == 1))
      {
       ...
       while (cur_mod_node != NULL)
         {
	  if (cur_mod_node->first_bus_node != NULL)
	     {
	       cur_bus_node = cur_mod_node->first_bus_node;
	       while (cur_bus_node != NULL)
		 {
                   fprintf(fp," %s",cur_bus_node->node_name);
		   cur_bus_node = cur_bus_node->next;
		 }
	     }
          else
	...
	if (inst_count == 0)
	  {
          ...
          if (inst_array_length > 0)
	    {
            inst_index = get_first_bus_val(cur_instance->name);
            inst_end = get_last_bus_val(cur_instance->name);
            if (inst_end > inst_index)
	      inst_inc = 1;
            else 
	      inst_inc = -1;
	    determine_base_name(cur_instance->name,inst_basename);
	    }
	  }
       ...
       if (cur_instance->mod->spice_text == NULL)
	 {
	   // print instance name
	  if (inst_array_length > 0)
             fprintf(fp,"%s__%d",inst_basename,inst_index);
          else
	  ...
	  while (cur_inst_node != NULL)
	    {
	      if (cur_inst_node->first_bus_node != NULL)
		{
		  if (inst_array_length > 0)
		    {
		     cur_bus_node = cur_inst_node->first_bus_node;
		     i = 0;
		     while (cur_bus_node != NULL)
		       {
			i++;
			if (i > inst_count)
		     ...
		     if (cur_bus_node == NULL)
		       {
			 printf("error in 'print_hspice_netlist': cur_bus_node is NULL!\n");
			 printf("  -> this should never happen!\n");
			 exit(1);
		       }
	             fprintf(fp," %s",cur_bus_node->node_name);
       ...
       else // turn primitive instance into a primitive
	 {
	  extract_primitive_string(cur_instance,primitive_string);
          for (i = 0; primitive_string[i] != '\0'; i++)
	    {
	      if (primitive_string[i] == '\\')
		primitive_string[i] = '\n';
	    }
	  fprintf(fp,"%s\n",primitive_string);
	 } 
       inst_count++;
       if (inst_count < inst_array_length)
          inst_index += inst_inc;
       else
	 {
          inst_count = 0;
    ...
    /////////// include special spice text from Sue2 icon view //////////
    if (cur_module->spice_module_instance_text != NULL)
      {
	fprintf(fp,"%s",cur_module->spice_module_instance_text);
	fprintf(fp,"\n");
      }
    if (cur_module->spice_module_include_text != NULL)
      {
	fprintf(fp,"%s",cur_module->spice_module_include_text);
	fprintf(fp,"\n");
      }
    ...
    if (cur_module->top_module_flag != 1 ||
       (cur_module->top_module_flag == 1 && print_top_subckt_flag == 1))
       fprintf(fp,".ends %s\n\n",cur_module->name);
    else if (cur_module->top_module_flag == 1)
       fprintf(fp,"\n");
...
MOD_PARAMETER *cur_mod_parameter;
char temp[MAX_ARG],primitive_string[MAX_TEXT];
int inst_array_length, inst_bus_length, inst_bundle_length;
int i;
...
while (cur_module != NULL)
  {
    // don't netlist primitives (i.e., transistors, resistors, etc.) as modules
    /***  allow spice primitives to be netlisted - MH Perrott, July 28, 2011
    if (cur_module->spice_text != NULL)
      {
      cur_module = cur_module->next;
      continue;
    ...
    ****/
    ...
    /**** Changed to using cppsim module to declare electrical_element instead of Sue2 file
    if (cur_module->spice_text != NULL)
    ...
    ****/
    ...
    while (cur_instance != NULL)
       ...
       if (cur_instance->mod == NULL)
	  {
       ...
       // print out non-primitive instances as is
       // changed this to allow spice primitives to be netlisted. MH Perrott July 29, 2011
       if (cur_instance->mod->spice_text == NULL || cur_instance->mod->spice_text != NULL)
	 {
	  // print instance name
	  ...
	  while (cur_inst_node != NULL)
	     ...
	     if (cur_mod_node == NULL)
		{
	     ...
	     inst_bus_length = get_bus_length(cur_inst_node->name);
             inst_bundle_length = get_bundle_length(cur_inst_node->name);
             if (inst_bus_length == 0 && inst_bundle_length == 0 && inst_array_length > 0)
	        {
    ...
    /////////// netlist global pins (utilize first_unused_instance) //////////
    ...
    while (cur_instance != NULL)
       ...
       if (cur_instance->mod == NULL)
	  {
       ...
       // print out only pins and globals
       /***
       ...
       ***/
       // print out only globals
       if (strcmp(cur_instance->mod->name,"global") == 0)
	 {
	  // print instance name
	  fprintf(fp,"instance: %s %s %s\n",cur_instance->name,
                   cur_instance->mod->name, cur_instance->mod->library_path);
...
while (cur_module != NULL)
   {
	 ...
	 // place top module at the beginning of the netlist
	 cur_module->sim_mod_order = -1;
       }
    cur_module = cur_module->next;
   }
return(top_module);
...
while(cur_module != NULL)
   {
	 ...
	 cur_instance->mod = grab_module(first_module,cur_instance->mod_name);
         if (cur_instance->mod == NULL)
	   {
	     printf("warning in 'connect_sue_instances_to_modules':\n");
...
// reset original case of arguments
extract_arguments(line_set,filename,linecount,args);
strncpy(instance->mod_name,args->arg_array[1],MAX_INSTANCE_NAME_LENGTH-1);
// back to lowercase for the remaining arguments
extract_arguments_lc(line_set,filename,linecount,args);
...
MODULE *add_sue_module(MODULE *first_module, const char *deck_filename, 
                       ...
                       int shared_library_flag)
{
return(add_sue_module(first_module, deck_filename, 
                       library_path, param_list,
                       shared_library_flag, "normal"));
...
int spice_module_include_text_flag, spice_module_instance_text_flag;
...
cur_module->shared_library_flag = shared_library_flag;
sprintf(cur_module->full_filename,"%s",deck_filename);
...
while(end_of_file_flag == 0)
  {
   ...
   if (strcmp(control_string,"lite") != 0)
     {
      if (start_icon_description_flag == 1)
	   ...
	   for (i = 1; i < args->num_of_args; i++)
           ...
           if (cur_node == NULL)
	     {
	       ...
	       printf("---> look in icon view of module '%s'\n",cur_module->name);
	       printf("line reads:\n");
               send_line_set_to_std_output(line_set);
	       exit(1);
	     }
	   ...
	   spice_text_description_flag = 0;
           spice_module_include_text_flag = 0;
           spice_module_instance_text_flag = 0;
                   ...
                   if (j < args->num_of_args-1)
		     {
                          ...
                          strcmp(args->arg_array[j+1],"spice") == 0)
			  {
			    spice_text_description_flag = 1;
                      ...
                      else if (strcmp(args->arg_array[i+1],"fixed") == 0 &&
                               strcmp(args->arg_array[j+1],"spice_module_include") == 0)
			  {
                           spice_module_include_text_flag = 1;
			  }
                      else if (strcmp(args->arg_array[i+1],"fixed") == 0 &&
                               strcmp(args->arg_array[j+1],"spice_module_instance") == 0)
			  {
                           spice_module_instance_text_flag = 1;
			  }
	   ...
	   else if (spice_module_include_text_flag == 1)
	     {
	       for (i = 1; i < args->num_of_args; i++)
		 {
		 if (strcmp(args->arg_array[i],"-text") == 0)
		   {
		     ...
		     for (j = i+1; j < args->num_of_args &&
			    args->arg_array[j][0] != '-'; j++)
		       {
		         ...
		         strcat(text_description," ");
                     ...
                     if ((cur_module->spice_module_include_text = 
                         (char *) malloc(sizeof(char[MAX_TEXT]))) == NULL)   
                         {   
                         printf("error in 'add_sue_module':  malloc call failed\n");   
                         printf("out of memory!\n");   
		     ...
		     strncpy(cur_module->spice_module_include_text,text_description,MAX_TEXT-1);
		     break;
		   }
		 }
	     }
	   else if (spice_module_instance_text_flag == 1)
	     {
	       for (i = 1; i < args->num_of_args; i++)
		 {
		 if (strcmp(args->arg_array[i],"-text") == 0)
		   {
		     ...
		     for (j = i+1; j < args->num_of_args &&
			    args->arg_array[j][0] != '-'; j++)
		       {
		         ...
		         strcat(text_description," ");
                     ...
                     if ((cur_module->spice_module_instance_text = 
                         (char *) malloc(sizeof(char[MAX_TEXT]))) == NULL)   
                         {   
                         printf("error in 'add_sue_module':  malloc call failed\n");   
                         printf("out of memory!\n");   
		     ...
		     strncpy(cur_module->spice_module_instance_text,text_description,MAX_TEXT-1);
		     break;
		   }
		 }
	     }
	   ...
	   {
	   // restore original case of arguments
           extract_arguments(line_set,deck_filename,linecount,args);
...
strcpy(cur_module->old_name,cur_module->name);
free(dummy_instance);
free(line_set);
free(args);
fclose(in);
...
strcpy(A->bus_node_name,"NULL");
A->wire_group = -1;
A->name_is_valid_flag = 0;
A->connecting_instance = NULL;
A->connecting_instance_terminal = NULL;
A->next = NULL;
...
if (first_bus_node == NULL)
  ...
  printf("bus nodes:\n");
cur_bus_node = first_bus_node;
while (cur_bus_node != NULL)
   {
     printf("%d: %s\n",count++,cur_bus_node->node_name);
     cur_bus_node = cur_bus_node->next;
   }
}
...
{
int i,j,k,m,ind1,ind2,count,flag,comma_flag,bus_multiplier;
BUS_NODE *first_bus_node,*cur_bus_node,*base_bus_node;
char cur_node_name[MAX_NODE_NAME_LENGTH];
char base_name[MAX_NODE_NAME_LENGTH];
char temp_name[MAX_NODE_NAME_LENGTH];
...
for (i = 0; node_name[i] != '\0'; i++)
   {
	 ...
	 for (  ; node_name[i] != '>' && node_name[i] != '\0'; i++)
	   {
	     if (node_name[i] < '0' || node_name[i] > '9')
	       {
   	        printf("error in 'evaluate_bus_nodes':\n");
                printf("    node name has inappropriate bus notation '<*N>node'\n");
	        printf("    -> in this case, node name is '%s'\n",node_name);
	        printf("    *** Please fix the netlist to use proper bus notation ***\n");
	        exit(1);
	       }
	   }
	 if (node_name[i] != '>')
	 ...
	 strcpy(temp_name,&node_name[m]);
	 temp_name[i-m] = '\0';
	 bus_multiplier = atoi(temp_name);
	 k = 0;
	 continue;
       }
     else if (node_name[i] == '<')
       {
	 cur_node_name[k] = '\0';
	 strcpy(base_name,cur_node_name);
         k = 0;
	 flag = 1;
	 count = 0;
       }
	 ...
	 if (flag != 1 || count > 0)
	   {
   	   printf("error in 'evaluate_bus_nodes':\n");
           printf("    node name has inappropriate bus notation 'node<N:M>'\n");
	   printf("    -> in this case, node name is '%s'\n",node_name);
	   printf("    *** Please fix the netlist to use proper bus notation ***\n");
	   exit(1);
	   }
	 ...
	 for (j = 0; cur_node_name[j] != '\0'; j++)
	   {
	     if (cur_node_name[j] < '0' || cur_node_name[j] > '9')
	       {
	       printf("error in 'evaluate_bus_nodes':\n");
               printf("    node name has inappropriate bus notation 'node<N:M>'\n");
	       printf("    -> in this case, node name is '%s'\n",node_name);
	 ...
	 ind2 = atoi(cur_node_name);

         if (count == 0)
	   {
	     for (m = 0; m < bus_multiplier; m++)
	       {
	       if (first_bus_node == NULL)
	          {
	           first_bus_node = init_bus_node(cur_node_name);
	           cur_bus_node = first_bus_node;
	          }
	       else
	          {
	           cur_bus_node->next = init_bus_node(cur_node_name);
	           cur_bus_node = cur_bus_node->next;
	          }
               sprintf(cur_bus_node->node_name,"%s<%d>",base_name,ind2);
	       }
	     bus_multiplier = 1;
	   }
	 else
		    ...
		    for (j = ind1; j >= ind2; j--)
		      {
	               if (first_bus_node == NULL)
	                  ...
	                  cur_bus_node = cur_bus_node->next;
                       ...
                       sprintf(cur_bus_node->node_name,"%s<%d>",base_name,j);
		      }
	          }
                else
	          {
	 ...
	 for (m = 0; m < bus_multiplier; m++)
	   {
	    if (first_bus_node == NULL)
	    ...
	    else
	      {
            ...
            sprintf(cur_bus_node->node_name,"%s",cur_node_name);
	   }
	 bus_multiplier = 1;
       }
     else if (node_name[i] != ' ')
         cur_node_name[k++] = node_name[i];
   }
 if (comma_flag == 1 && k > 0)
   {
    cur_node_name[k] = '\0';
...
if ((A = (BUS_NODE *) malloc(sizeof(BUS_NODE))) == NULL)   
   {   
   printf("error in 'init_bus_node':  malloc call failed\n");   
   printf("out of memory!\n");   
   exit(1);   
   }   

strcpy(A->node_name,node_name);
A->next = NULL;
...
char *retrieve_instance_parameter_val(INSTANCE *cur_instance, const char *param_name)
{
MOD_PARAMETER *cur_mod_parameter,*cur_inst_parameter;
char temp[MAX_ARG];
...
strncpy(A->old_name,name,MAX_MODULE_NAME_LENGTH-1);
...
A->spice_text = NULL;
A->spice_module_include_text = NULL;
A->spice_module_instance_text = NULL;
return(A);   
}
...
strcpy(cur_node->mod_node_name,cur_mod_node->name);
...
double eval_paren_expr(const char *input_in, const char *file_name, LINE *line_set, 
                       PARAMETER *par_list)
{
...
double eval_no_paren_expr(const char *input, const char *file_name, LINE *line_set,
                          PARAMETER *par_list)
{
...
{
int i,j,flag;
char arg1[MAX_ARG],arg2[MAX_ARG],in[MAX_ARG];
int arg1_val, arg2_val;

if (strcmp(in_raw,"NULL") == 0)
   return(0);
 
strcpy(in,in_raw);
replace_angle_brackets_with_square_brackets(in);
...
for (i = 0, j = 0; in[i] != '\0'; i++)
   {
    if (in[i] == '[')
       flag = 1;
    else if (flag == 1 && in[i] != ':' && in[i] != ']' && in[i] != ' ')
       arg1[j++] = in[i];
    else if (in[i] == ':')
      {
	if (flag == 0)
	  {
          printf("error in 'determine_first_bit': did not find initial '[' character\n");
          printf("  expression reads:  '%s'\n",in);
          exit(1);
	  }
        else
	  {
...
if (flag == 0)
   ...
   {
     printf("error in 'determine_first_bit': did not find ':' character\n");
     printf("  expression reads:  '%s'\n",in);
     exit(1);
   }
else if (flag == 2)
...
for (i = 0, j = 0; in[i] != '\0'; i++)
   {
	...
	if (flag == 0)
	  {
          printf("error in 'determine_last_bit': did not find initial '[' character\n");
          printf("  expression reads:  '%s'\n",in);
          exit(1);
	  }
        else
...
if (flag == 0)
   ...
   {
     printf("error in 'determine_last_bit': did not find ':' character\n");
     printf("  expression reads:  '%s'\n",in);
     exit(1);
   }
else if (flag == 2)
...
flag = 0;
for (i = 0; inst_name[i] != '\0'; i++)
   {
     if (flag == 0)
        base_name[i] = inst_name[i];
...
if (flag == 0)
  {
   ...
   {
     printf("error in 'check_if_arrayed_signal_name':\n");
     printf("   mismatch in '<' and '>' in signal name\n");
     printf("   signal name is: '%s'\n",inst_name);
     exit(1);
   }
else if (flag == 2)
  {
...
return(-1); // will never reach here, but need to include to avoid C++ compile warning
}
...
for (i = 0, j = 0; in[i] != '\0'; i++)
   {
	...
	if (flag == 0)
	  {
          printf("error in 'determine_max_bit': did not find initial '[' character\n");
          printf("  expression reads:  '%s'\n",in);
          exit(1);
	  }
        else
...
if (flag == 0)
   ...
   {
     printf("error in 'determine_max_bit': did not find ':' character\n");
     printf("  expression reads:  '%s'\n",in);
     exit(1);
   }
else if (flag == 2)
...
for (i = 0, j = 0; in[i] != '\0'; i++)
   {
	...
	if (flag == 0)
	  {
          printf("error in 'determine_min_bit': did not find initial '[' character\n");
          printf("  expression reads:  '%s'\n",in);
          exit(1);
	  }
        else
...
if (flag == 0)
   ...
   {
     printf("error in 'determine_min_bit': did not find ':' character\n");
     printf("  expression reads:  '%s'\n",in);
     exit(1);
   }
else if (flag == 2)
