...
if (max < 1e-20)
   {
     printf("Error in Poly.set (using List):  all elements within\n");
     printf("  the input list appear to be zero!\n");
     exit(1);
   }
max *= 1.0e-12;
...
if (length < 1)
  {
   printf("error in Poly.set (using Vector):  vector length is zero\n");
   exit(1);
  }
if (num_coeff != 0)
   {
...
if (max < 1e-20)
   {
     printf("Error in Poly.set (using Vector):  all elements within\n");
     printf("  the input vector appear to be zero!\n");
     exit(1);
   }
max *= 1.0e-12;
...
if ((coeff = (double *) calloc(num_coeff,sizeof(double))) == NULL)
   {
    printf("Error in Poly.set (using Vector): out of memory for coeff calloc\n");
    exit(1);
   }
if ((exp = (double *) calloc(num_coeff,sizeof(double))) == NULL)
   {
    printf("Error in Poly.set (using Vector): out of memory for exponent calloc\n");
    exit(1);
   }
...
if (prev_in_array == NULL)
    {
      printf("Error in 'EdgeInterpolate::get_prev_sample_vec()':\n");
      printf("   previous vector value is not defined!\n");
      printf("   -> be sure to call 'EdgeInterpolate::update()' first\n");
...
if (prev_in_array == NULL)
    ...
    if ((prev_in_array = (double *) calloc(vec_length,sizeof(double))) == NULL)
       { 
        printf("Error in 'EdgeInterpolate::inp': out of memory for calloc\n");
        exit(1);
       }
    }
if (vec_out.get_length() != vec_length)
...
if (prev_in_array == NULL)
    ...
    if ((prev_in_array = (double *) calloc(vec_length,sizeof(double))) == NULL)
       { 
        printf("Error in 'EdgeInterpolate::update': out of memory for calloc\n");
        exit(1);
       }
    }
...
if ((fp = fopen(filename,"wb")) == NULL)
   {
    printf("Error in Probe64 Construct:  file '%s' can't be opened\n",filename);
    exit(1);
   }
if (subsample < 1)
   {
    printf("Error in Probe64 Construct:  subsample must be > 0\n");
    printf("  in this case, subsample = %d\n",subsample);
    exit(1);
   }
sample_period = sample_per; 
     ...
     if (strcmp(node_name,first_node->name) == 0)
       {
        if (fp == NULL)
	 {
	   printf("error in Probe64.inp - filename has not been specified!\n");
	   exit(1);
	 }
...
if (header_flag == 1)
  {
   ...
   if (strcmp(node_name,cur_node->name) != 0)
     {
       printf("error in Probe64.inp - the signal probed does not match\n");
       printf("   with the signal that should be specified\n");
       printf("      the node that should be probed is: %s\n", cur_node->name);
       printf("      the node that is being probed is: %s\n", node_name);
       printf("   this error may be caused by using the same probe object\n");
...
if (flush_buffer == 1)
  {
   ...
   if (fwrite(dbuf, sizeof(double),buf_length,fp) != buf_length)    
      {
       printf("error using 'create_data64': out of disk space\n");
       exit(1);
      }
   ibuf[0] = buf_length*8;
   fwrite_int_values(ibuf,1,fp);
...
if (cur_node == NULL)
   {
    printf("error in 'create_header64':  first_node is NULL\n");
    exit(1);
   }
num_signals = 1;
while(cur_node->next != NULL)
...
if (index < 0)
   {
    printf("error in 'Vector::add_to_elem':  index must be >= 0\n");
    printf("  in this case, index = %d\n",index);
    printf("  and you ran add_to_elem() on vector '%s'\n",name);
    printf("     (originating module of vector = '%s')\n",module_name);
    exit(1);
   }
...
if (index >= vec->length)
    {
    printf("error in 'Vector::add_to_elem':  index must be < length of vector\n");
    printf("  in this case, index = %d, vector length = %d\n",index,vec->length);
    printf("  and you ran add_to_elem() on vector '%s'\n",name);
    printf("     (originating module of vector = '%s')\n",module_name);
    exit(1);
...
if (index < 0)
   {
    printf("error in 'IntVector::add_to_elem':  index must be >= 0\n");
    printf("  in this case, index = %d\n",index);
    printf("  and you ran add_to_elem() on vector '%s'\n",name);
    printf("     (originating module of vector = '%s')\n",module_name);
    exit(1);
...
if (index >= vec->length)
    {
    printf("error in 'IntVector::add_to_elem':  index must be < length of vector\n");
    printf("  in this case, index = %d, vector length = %d\n",index,vec->length);
    printf("  and you ran add_to_elem() on vector '%s'\n",name);
    printf("     (originating module of vector = '%s')\n",module_name);
    exit(1);
...
if (index_row < 0 || index_col < 0)
   {
    printf("error in 'Matrix::add_to_elem':  row and column index must be >= 0\n");
    printf("  in this case, index_row = %d, index_col = %d\n",index_row,
             index_col);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
    exit(1);
   }
...
if (index_row >= mat->rows)
    {
    printf("error in 'Matrix::add_to_elem':  index_row must be < rows of matrix\n");
    printf("  in this case, index_row = %d, matrix is %d by %d\n",index_row,
               mat->rows, mat->cols);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
...
if (index_col >= mat->cols)
    {
    printf("error in 'Matrix::add_to_elem':  index_col must be < cols of matrix\n");
    printf("  in this case, index_col = %d, matrix is %d by %d\n",index_col,
               mat->rows, mat->cols);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
...
if (index_row < 0 || index_col < 0)
   {
    printf("error in 'IntMatrix::add_to_elem':  row and column index must be >= 0\n");
    printf("  in this case, index_row = %d, index_col = %d\n",index_row,
             index_col);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
...
if (index_row >= mat->rows)
    {
    printf("error in 'IntMatrix::add_to_elem':  index_row must be < rows of matrix\n");
    printf("  in this case, index_row = %d, matrix is %d by %d\n",index_row,
               mat->rows, mat->cols);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
...
if (index_col >= mat->cols)
    {
    printf("error in 'IntMatrix::add_to_elem':  index_col must be < cols of matrix\n");
    printf("  in this case, index_col = %d, matrix is %d by %d\n",index_col,
               mat->rows, mat->cols);
    printf("  and you ran add_to_elem() on matrix '%s'\n",name);
    printf("     (originating module of matrix = '%s')\n",module_name);
...
if (min <= INV_DP_SV_L*max)   
   {   
    printf("error in 'inv(A,B)':  input matrix A ('%s') is singular!\n",   
               A->name);   
...
if ((A = (MexListStreamFrame *) calloc(1,sizeof(MexListStreamFrame))) == NULL)   
   {   
   printf("error in 'init_mex_list_stream_frame':  calloc call failed\n");   
   printf("out of memory!\n");   
   printf("Error: need to reduce length of CppSim mex frame\n");
   exit(1);
  }
A->length = 0;
A->next = NULL;
...
if (f_corner < 0.0)
  {
   printf("Error in OneOverfPlusWhiteNoise::OneOverfPlusWhiteNoise:  f_corner must be >= 0.0\n");
   printf("  in this case, f_corner = %5.3e\n",f_corner);
   exit(1);
  }
if (a <= 0.01)
  {
   printf("Error in OneOverfPlusWhiteNoise::OneOverfPlusWhiteNoise:  a must be > 0.01\n");
   printf("  in this case, a = %5.3e\n",a);
   exit(1);
  }
if (r <= 0.01)
  {
   printf("Error in OneOverfPlusWhiteNoise::OneOverfPlusWhiteNoise:  r must be > 0.01\n");
   printf("  in this case, r = %5.3e\n",r);
   exit(1);
  }
if (Ts < 1e-30)
  {
   printf("Error in OneOverfPlusWhiteNoise::OneOverfPlusWhiteNoise:  Ts must be >= 1e-30\n");
   printf("  in this case, Ts = %5.3e\n",Ts);
   exit(1);
  }
...
if (slope > -5)
  {
   printf("Error in OneOverfPlusWhiteNoise::OneOverfPlusWhiteNoise:  slope must be between -5 and -15\n");
   printf("  in this case, slope = %5.3g\n",slope);
   exit(1);
  }
if (slope < -15)
  {
...
if (f_corner < 0.0)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  f_corner must be >= 0.0\n");
   printf("  in this case, f_corner = %5.3e\n",f_corner);
   exit(1);
  }
...
if (a <= 0.01)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  a must be > 0.01\n");
   printf("  in this case, a = %5.3e\n",a);
   exit(1);
  }
if (r <= 0.01)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  r must be > 0.01\n");
   printf("  in this case, r = %5.3e\n",r);
   exit(1);
  }
if (Ts < 1e-30)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  Ts must be >= 1e-30\n");
   printf("  in this case, Ts = %5.3e\n",Ts);
   exit(1);
  }
...
if (f_corner < 0.0)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  f_corner must be >= 0.0\n");
   printf("  in this case, f_corner = %5.3g\n",f_corner);
   exit(1);
  }
if (Ts < 1e-30)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  Ts must be >= 1e-30\n");
   printf("  in this case, Ts = %5.3g\n",Ts);
   exit(1);
  }

if (slope > -5)
  {
   printf("Error in OneOverfPlusWhiteNoise::set:  slope must be between -5 and -15\n");
   printf("  in this case, slope = %5.3g\n",slope);
   exit(1);
  }
if (slope < -15)
...
if (no_decay_flag == 0)
   ...
   if (Pnode == NULL)
      {
       printf("error in 'NeuralNode::inp':  Pnode is NULL!\n");
       exit(1);
      }
...
void NeuralNode::set_node_pointer(NeuralNode *Pnode_in, int num_nodes_in)
{
  if (num_nodes_in < 0)
    {
      printf("error in 'NeuralNode::set_node_pointer':\n");
      printf("  num_nodes = %d, but must be > 0\n",num_nodes_in);
      exit(1);
    }
Pnode = Pnode_in;
...
void NeuralNode::set_tau_and_gain(double tau, double gain, double Ts)
{
  ...
  if (tau < 1e-12)
    {
      printf("error in 'NeuralNode::set_tau_and_gain':\n");
      printf("   tau must be > 1e-12\n");
      printf("   in this case, tau = %5.3e\n",tau);
      exit(1);
    }
...
void NeuralNode::add_input(int node_index, double scale_factor)
{
  ...
  if (node_index < 0 || node_index >= num_nodes)
    {
      printf("error in 'NeuralNode:add_input':  node_index out of range\n");
      printf("   in this case, node_index = %d\n", node_index);
      printf("   but must instead be >= 0 and < %d\n",num_nodes);
      exit(1);
    }
...
if (Ts_in < 1e-30)
   {
    printf("error in 'Neuron_Zone_Element::set_filter':\n");
    printf("  the value of 'Ts' should be >= 1e-30\n");
    printf("  -> in this case, Ts = %5.3e\n",Ts_in);
    exit(1);
   }
Ts = Ts_in;
...
if (fp < 1e-12)
   {
    printf("error in 'Neuron_Zone_Element::set_filter':\n");
    printf("  the value of 'fp' should be >= 1e-12\n");
    printf("  -> in this case, fp = %5.3e\n",fp);
    exit(1);
   }
if (fz < 1e-12 && fz >= 0.0)
   {
    printf("error in 'Neuron_Zone_Element::set_filter':\n");
    printf("  the value of 'fz' should be >= 1e-12\n");
    printf("  or less than 0 (indicating zero should be removed)\n");
    printf("  -> in this case, fz = %5.3e\n",fz);
    exit(1);
   }
if (saturation < 0.0)
   {
    printf("error in 'Neuron_Zone_Element::set_filter':\n");
    printf("  the value of 'saturation' should be >= 0.0\n");
    printf("  -> in this case, saturation = %5.3e\n",saturation);
    exit(1);
   }
...
if (gain1_in < 0.0)
    {
    printf("error in 'Neuron_Zone_Element::set_gain':\n");
    printf("  the value of 'gain1' should be >= 0.0\n");
    printf("  -> in this case, gain1 = %5.3e\n",gain1_in);
    exit(1);
   }
if (gain2_in < 0.0)
    {
    printf("error in 'Neuron_Zone_Element::set_gain':\n");
    printf("  the value of 'gain2' should be >= 0.0\n");
    printf("  -> in this case, gain2 = %5.3e\n",gain2_in);
    exit(1);
   }
gain1 = gain1_in;
gain2 = gain2_in;
...
if (gain2_in < 0.0)
    {
    printf("error in 'Neuron_Zone_Element::set_auxiliary_gain':\n");
    printf("  the value of 'gain2' should be >= 0.0\n");
    printf("  -> in this case, gain2 = %5.3e\n",gain2_in);
    exit(1);
   }
...
if (phase1 < prev_phase1)
    {
      ...
      if (temp_calc < 1e-12)
	{
	  printf("error in 'Neuron_Zone_Element::inp':\n");
          printf("  -> phase1 input is corrupted!\n");
          printf("     difference between current and prev phase is\n");
          printf("     to small!\n");
          printf("     cur_phase1 = %5.3e, prev_phase1 = %5.3e\n",
		 phase1, prev_phase1);  
          exit(1);
	}
      ...
      if (phase_count1 < 1.0)
	{
	  printf("error in 'Neuron_Zone_Element::inp':\n");
          printf("  -> phase1 input is corrupted!\n");
          printf("     phase wrap occured in less than one sample!\n");
          printf("     phase_count1 = %5.3e\n",phase_count1);
          exit(1);
	}
...
if (phase2 < prev_phase2)
	...
	{
	  printf("error in 'Neuron_Zone_Element::inp':\n");
          printf("  -> phase2 input is corrupted!\n");
          printf("     difference between current and prev phase is\n");
          printf("     to small!\n");
          printf("     cur_phase2 = %5.3e, prev_phase2 = %5.3e\n",
		 phase2, prev_phase2);  
          exit(1);
	}
...
if (Ts < 1e-30)
   {
      printf("error in 'Neuron_Firing_Element::set_nom_freq_and_kv':\n");
      printf("  must first run 'Neuron_Firing_Element::set_time_step'\n");
      printf("  to set the value of Ts\n");
      exit(1);
   }
...
if (nom_freq < 1e-12)
    {
      printf("error in 'Neuron_Firing_Element::set_nom_freq_and_kv':\n");
      printf("  nom_freq must be >= 1e-12\n");
      printf("  -> in this case, nom_freq = %5.3e\n",nom_freq);
      exit(1);
    }
Kv = Kv_in;
if (Kv < 0.0)
    {
      printf("error in 'Neuron_Firing_Element::set_nom_freq_and_kv':\n");
      printf("  Kv must be >= 0.0\n");
      printf("  -> in this case, Kv = %5.3e\n",Kv);
      exit(1);
    }
}
...
if (phase < 0.0 || phase >= 1.0)
    {
      printf("error in 'Neuron_Firing_Element::set_phase':\n");
      printf("  phase must be >= 0.0 and < 1.0\n");
      printf("  -> in this case, phase = %5.3e\n",phase);
      exit(1);
    }
out = phase;
}
...
if (Ts < 1e-30)
    {
      printf("error in 'Neuron_Firing_Element::set_time_step':\n");
      printf("  Ts must be >= 1e-30\n");
      printf("  -> in this case, Ts = %5.3e\n",Ts);
      exit(1);
    }
}
...
if (nom_freq < 0.0)
    {
      printf("error in 'Neuron_Firing_Element::inp':\n");
      printf("  must set 'nom_freq' and 'Kv' by running\n");
      printf("  method 'set_nom_freq_and_kv(nom_freq,Kv)'\n");
      exit(1);
    }
if (Ts < 1e-30)
   {
...
if (num_neurons < 1)
    {
      printf("error in 'Neuron_Element4::store_PNeuron':\n");
      printf("   num_neurons must be > 0\n");
      printf("   -> in this case, num_neurons = %d\n",num_neurons);
      exit(1);
    }
if (PNeuron == NULL)
   {
      printf("error in 'Neuron_Element4::store_PNeuron':\n");
      printf("   PNeuron is NULL!\n");
      exit(1);
   }
}
...
if (num_inputs < 1)
    {
      printf("error in 'Neuron_Element4::store_input_vec':\n");
      printf("   num_inputs must be > 0\n");
      printf("   -> in this case, num_inputs = %d\n",num_inputs);
      exit(1);
    }
if (input_vec == NULL)
   {
      printf("error in 'Neuron_Element4::store_input_vec':\n");
      printf("   input_vec is NULL!\n");
      exit(1);
   }
if (num_inputs != input_vec->get_length())
   {
      printf("error in 'Neuron_Element4::store_input_vec':\n");
      printf("   num_inputs does not match input vector length!\n");
      printf("   -> num_inputs = %d, input_vec length = %d\n",
...
if (zone_num >= num_zones || zone_num < 0)
    {
      printf("error in 'Neuron_Element4::set_zone_gain':\n");
      printf("   zone_num must be >= 0 and < num_zones\n");
      printf("   -> in this case: zone_num = %d, num_zones = %d\n",
	     zone_num, num_zones);
      exit(1);
    }
...
if (zone_num >= num_zones || zone_num < 0)
    {
      printf("error in 'Neuron_Element4::set_zone_coupling_gain':\n");
      printf("   zone_num must be >= 0 and < num_zones\n");
      printf("   -> in this case: zone_num = %d, num_zones = %d\n",
	     zone_num, num_zones);
      exit(1);
...
if (src_zone_num >= num_zones)
    {
      printf("error in 'Neuron_Element4::set_zone_coupling_gain':\n");
      printf("   src_zone_num must be < num_zones\n");
      printf("   -> in this case: src_zone_num = %d, num_zones = %d\n",
...
if (src_zone_num < 0)
   {
      printf("error in 'Neuron_Element4::set_zone_coupling_gain':\n");
      printf("   src_zone_num must be >= 0\n");
      printf("   -> in this case: src_zone_num = %d\n", src_zone_num);
      exit(1);
   }
...
if (zone_num >= num_zones || zone_num < 0)
    {
      printf("error in 'Neuron_Element4::set_zone_filter':\n");
      printf("   zone_num must be >= 0 and < num_zones\n");
      printf("   -> in this case: zone_num = %d, num_zones = %d\n",
	     zone_num, num_zones);
      exit(1);
...
if (fp < 1e-12)
    {
      printf("error in 'Neuron_Element4::set_thresh_and_bw':\n");
      printf("  fp must be >= 1e-12\n");
      printf("  -> in this case, fp = %5.3e\n",fp);
      exit(1);
    }
if (Ts < 1e-30)
   {
      printf("error in 'Neuron_Element4::set_thresh_and_bw':\n");
      printf("  must first run 'Neuron_Element4::set_zone_filter'\n");
      printf("  to set the value of Ts\n");
      exit(1);
   }
...
if (Ts < 1e-30)
   {
      printf("error in 'Neuron_Element4::set_firing_parameters':\n");
      printf("  must first run 'Neuron_Element4::set_zone_filter'\n");
      printf("  to set the value of Ts\n");
      exit(1);
   }
...
if (zone_num >= num_zones || zone_num < 0)
    {
      printf("error in 'Neuron_Element4::set_node_connection_to_pnode':\n");
      printf("   zone_num must be >= 0 and < num_zones\n");
      printf("   -> in this case: zone_num = %d, num_zones = %d\n",
	     zone_num, num_zones);
      exit(1);
...
if (neuron_index >= num_neurons || neuron_index < 0)
   {
      printf("error in 'Neuron_Element4::set_node_connection_to_pnode':\n");
      printf("   neuron_index must be >= 0 and < num_neurons\n");
      printf("   -> in this case: neuron_index = %d, num_neurons = %d\n",
...
if (zone_num >= num_zones || zone_num < 0)
    {
      printf("error in 'Neuron_Element4::set_node_connection_to_input':\n");
      printf("   zone_num must be >= 0 and < num_zones\n");
      printf("   -> in this case: zone_num = %d, num_zones = %d\n",
	     zone_num, num_zones);
      exit(1);
...
if (input_index >= num_inputs || input_index < 0)
   {
      printf("error in 'Neuron_Element4::set_node_connection_to_input':\n");
      printf("   input_index must be >= 0 and < num_inputs\n");
      printf("   -> in this case: input_index = %d, num_inputs = %d\n",
...
if (num_zones > 4)
   {
     printf("error in 'Neuron_Element4':  only 4 zones assumed\n");
     printf("   here:   num_zones = %d\n",num_zones);
     exit(1);
   }
for (i = 0; i < num_zones; i++)
   {
...
if (expr_in[i] != '\0')
  {
   printf("error in EvalExpression.eval:  input expression is too long\n");
   printf("  string is limited to MAX_CHAR_LENGTH = %d\n",MAX_CHAR_LENGTH);
   printf("   must increase MAX_CHAR_LENGTH in source code\n");
   printf("   if you want to use the following input expression:\n");
   printf("   expr_in = %s\n",expr_in);
...
while(p != NULL)
   {
    if (i >= 10)
      {
	printf("error in EvalExpression.eval:  number of parameters exceeded\n");
	printf("  (it's limited to 10 parameters");
	printf("  arg string: %s\n",vars);
	printf("  associated expression: %s\n",expr_in);
	exit(1);
     ...
     if (i == 0)
       ...
       if (var0 == NaN)
	 {
          printf("error in EvalExpression.eval:  value of variable '%s' not set\n",
              p);
	  printf("   overall expression: %s\n",expr_in);
	  exit(1);
	 }
 ...
 while(cur_var != NULL)
   {
     if (cur_var->is_valid_var_flag == 0)
       {
	 printf("error in 'EvalExpression.eval':  variable '%s' is undefined\n",cur_var->name);
         printf("   for expr: '%s'\n",expr_in);
	 exit(1);
       }
...
if (i != 1)
   {
     print_arg_list();
     printf("error in 'EvalExpression.eval':  expression not evaluated properly!\n");
     printf("  -> expr = '%s'\n",expr_in);
     exit(1);
   }
out = first_arg->value;
...
if (flag == 0)
   {
     printf("error in 'EvalExpression.set_var_value':\n");
...
if (first_var != NULL)
   {
     printf("error in 'EvalExpression.determine_variables':  first_var is != NULL!\n");
     exit(1);
   }
cur_var = first_var;
cur_arg = first_arg;
...
if (first_arg != NULL)
   {
     printf("error in 'EvalExpression.parse_into_args': first_arg != NULL!\n");
     exit(1);
   }
cur_arg = first_arg;
...
if (Ts_value < 0.0)
  {
    if (Ts_val < 1e-30)
      {
	printf("error in 'cppDigTransitions::incr_sample_step()':  Ts_val < 1e-30!\n");
	printf("  -> in this case, Ts_val = %5.3e\n",Ts_val);
	exit(1);
      }
    Ts_value = Ts_val;
